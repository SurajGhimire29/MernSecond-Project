...
const Navbar = () => {
  ...
  const { user, logout } = useAuth(); // no isAdmin
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-6">
            <Link to="/cart" className="relative">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-amber-600 text-white text-xs font-medium rounded-full w-5 h-5 flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </Link>
            ...
            {user ? (
              <div className="relative group">
                <button className="flex items-center gap-2">
                  ...
                  <span className="hidden md:inline text-sm text-gray-700">{firstName}</span>
                </button>
                <div className="absolute right-0 mt-3 w-48 bg-white rounded-lg shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                  <div className="py-2">
                    <Link
                      to="/profile"
